...
Let's implement a very simple tag called `l-cm: <player-name/>`, starting out by defining a parameter
...
For this exact reason, the renderer allows to interpret custom markup at the time of
...
```component-markup
<gray>Hello, <player-name renderer={ <rainbow>{player_name} } />! :)
```
