...
As the main use-case of this tag is to make color-theming easier, special shorthand-notation
has been introduced, where any attribute-name other than `value` is supported, as long as `value`
itself is absent and there is exactly one attribute specified; this fact allows for some neat
by-name binding, making for more precise markup.
