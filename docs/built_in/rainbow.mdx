...
Colorizes enclosed content with one continuous rainbow char-by-char, where client-rendered content like the [Keybind](./keybind.md)-Tag and its companions cannot, due to inherent limitations, be split up and will just be considered as an atomic unit.

There are many available attributes, like whether to override the color of already-colorized members, whether to skip whitespace (for better visual continuity), whether to skip non-text (aforementioned atomic units), whether to merge inner rainbows in order to create one large continuous transition as well as whether to dive into markup-attributes of children to colorize them too (deep).
