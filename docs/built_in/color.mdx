...
If the color is to be driven by data, or when values shall be fully custom (for various purposes,
like modded clients, etc.), there exists the explicit tag which takes in a `value`-attribute.

```component-markup
<color value="static value">My text</>
```

```component-markup
<color [value]="<expression>">My text</>
```