...
For convenience, the `key` attribute also supports a bound expression of arbitrary name,
while all other attributes support bound expressions with matching names.

```!component-markup
<container +let-my_key="block.minecraft.red_concrete">
  You received <&b>16x <translate [my_key]/>
```