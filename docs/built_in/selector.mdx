...
Fetches one or more entities based on a [Selector-Expression](../types#selector), which is always relative to the recipient of the message. The `renderer` is handed an implicit environment-variable called `selector_result` which is a list of type [PlatformEntity](../types#platformentity), as well as a `selector_origin`, which is the recipient of the message, also a `PlatformEntity`.
...
<img className='rendered-image' style={{width: '100%'}} src={useBaseUrl('/img/selector_plain.png')} />
...
<img className='rendered-image' style={{width: '100%'}} src={useBaseUrl('/img/selector_teleport.gif')} />