...
In the above example, the attribute `my-attribute` of the tag `my-tag` will be instantiated once for each item of the [Immediate List](./expression_syntax#immediate-list) `l-me: ['hello', 'world', 'test']`, such that the resulting meaning effectively becomes:
