...
in any way that is visually appealing to work with. In order to inject [Linebreaks](./built_in/linebreak)
or surrounding [Spaces](./built_in/space), please check out the corresponding tags as linked to learn
...
If the value of a placeholder-expression evaluates to `l-me: null`, no value will be substituted, allowing for convenient conditional displaying.
...
This powerful feature allows to define reusable templates which then may be injected and augmented
(conditions, loops, etc.) as needed; when evaluating said templates, they are granted access to
all environment-variables and bindings which are available at the scope of their placeholder's position.
Check out the examples of the [Container](./built_in/container.md)-tag (especially parameterization)
to learn more about the details.

Next to instances of `MarkupNode`, platform-specific `Component`-objects may also be injected at every
placeholder-position, allowing for seamless interoperability with existing systems (or pre-evaluated
markup) via environment-variables.

## Tags

Tags represent calls to internally existing functionality, which then can modify and or generate content dynamically; see [Built-In Tags](./category/built-in-tags) to discover predefined features and check out the [Extension API](./extension_api) as to learn about creating your own. There are two types of notation: one being comprised of an opening- and a (optional) closing-tag, while the other self-closes within the opening-tag and thereby does not bear any content.
...
- `a` is of type string (sequence of characters) with double-quotes
- `b` is also a string, this time with single-quotes (use as needed, e.g. to avoid having to escape quotes)
- `c` is a so-called template-literal using string-interpolation
- `d` of type whole number
- `e` of type fractional number
- `f` of type flag-style boolean (implicit `l-me: true`-value)
- `g` also of type boolean (`l-me: false`-value)
- `h` a `l-me: null`-value and
- `i` being a markup-value, delimited by curly-brackets.
...
In the above example, the attribute `my-attribute` of the tag `my-tag` will be instantiated once for each item of the [Immediate Array](./expression_syntax#immediate-array) `l-me: ['hello', 'world', 'test']`, such that the resulting meaning effectively becomes:
...
since it assigns one list-item to one instance of the `with`-attribute, due to the spread-operator being present.
...
For the sake of convenience, all expressions that consist either of a single identifier or a
member-access-chain (i.e. `l-me: a.b.c.d`; the identifier at the very end of the chain makes
for the name used in the shorthand binding) may be bound directly by dropping the assigned
value altogether: `[my_attribute]` is equivalent to `[my-attribute]="my_attribute"`. Due to
the fact that markup-attributes use kebab-case while expressions use snake_case, the
conversion between the two styles is carried out automatically.

Many tags also support an arbitrarily named, directly bound dynamic attribute, in order to
supply their main value (often `value`, `key`, etc.; check individual docs for details), which
once again allows for very terse notation.

The below is a mere example, where all let-bindings are meant to be environment-variables:

```!component-markup
<container
  *let-first={<green>first}
  *let-second={<gold>second}
  +let-my_key="chat.type.advancement.challenge"
  *let-first_wrapper="{with: first}"
  *let-second_wrapper="{with: second}"
>
  <translate
    [my_key]
    [first_wrapper.with]
...
The simplest form of a conditional is represented by the intrinsic `*if` attribute, which is **always** treated as an expression (square brackets must **not** be added explicitly); if matching, the tag to which it is applied shows up and if not, the tag will not be evaluated.
...
```component-markup
<red *if="my_number eq 1">Shown if the value is one!</>
<red *else-if="my_number eq 2">Shown if the value is two!</>
<red *else-if="my_number eq 3">Shown if the value is three!</>
...
```component-markup
<red *if="my.input.expression eq 'A'">First case</>
<green *else-if="my.input.expression eq 'B'">Second case</>
<blue *else-if="my.input.expression eq 'C'">Third case</>
...
The following demonstrates a conditionally applied rainbow.

```!!component-markup
<rainbow *use="render_count % 2 eq 0">Hello, world!</> ({render_count})
```
...
When there's no need to access the current item of iteration, simply drop the dash and use the attribute as-is; this shorter style is especially useful in combination with [Ranges](./expression_syntax#range-operator) (the `l-me: loop` context is discussed further below).

```!component-markup
<red *for="1..5" *for-separator={<br/>}>The #{loop.index + 1} occurrence of this line!
```
...
<gray>
  ...
  <container
    ...
    *for-separator={<gray>, }
  >
    <green *if="player_name eq 'Steve'">Alternate Name Here</>
...
In order to provide first-class support for reusable components, which also drastically simplifies
trying to extract common patterns or provid users with a way to quickly define custom color-schemes,
tag-names starting with the dollar-sign `$` have been reserved for template substitution.

Following said dollar-sign, an expression is expected, which is evaluated within the currently provided
environment and then interpreted as a markup-node. Seeing how substituted components are evaluated in
the same scope as their original placeholder resided in, they also gain access to all variables of the
current scope. These placeholder-tags may be self-closing (no children)

```!component-markup
<container
  *let-my_value={<red>A simple, static value.}
>
  <$my_value *for="1..3" *for-separator={<br/>}/>
```

or can even hold content, which will be available to the substituted template via the `l-cm: <content/>`-tag

```!component-markup
<container
  *let-pri={<gray><content/>}
  *let-sec={<aqua><bold><content/>}
>
  <$pri>This part is <$sec>highlighted</> in a unique way!
```

The above syntax allows for very succinct composition of styles, without having to make use of
let-bindings as to introduce intermediate components to be passed via the temporary environment;
instead, the `l-cm: <content/>`-tag directly provides access, encouraging substitution thanks to simplicity.

For convenience, all attributes on a template-substitution tag are introduced as let-bindings into
the scope of the substituted template, allowing for what reads almost like a function-call. Just remember
that attributes use kebab-case, which is internally converted to snake_case before introducing the binding.

```!component-markup
<container
  *let-my_template={<&b>Hello, {first_name} {last_name}!}
>
  <$my_template first-name="Markus" last-name="Persson" />
```
