...
In the example above, `a` is of type string (sequence of characters), `b` of type whole number, `c` of type fractional number, `d` of type flag-style boolean (implicit `l-me: true`-value), `e` also of type boolean (`l-me: false`-value), `f` a `l-me: null`-value and `g` being a markup-value, delimited by curly-brackets. In general, the only differentiation occurs between markup-values and scalar-values (string, number, flag), where the latter will never cause a type-mismatch and simply be interpreted as required.
