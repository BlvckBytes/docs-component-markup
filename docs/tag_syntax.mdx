...
in order to improve runtime performance - especially in comparison to the `*if`-equivalent.
...
If no specified `*is`-case applies, an optional `*else`-fallback may be provided to evaluate; if absent, nothing
...
are required to be members of the matching-process, meaning `*is`-cases or an `*else`, where the latter may only
