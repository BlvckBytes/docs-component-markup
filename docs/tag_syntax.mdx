...
which will display a value of `343`. Next to expressions, markup-values may also be bound to temporary variables, effectively allowing to define reusable components.
...
If bindings containing literal strings are to be created, simply employ the literal intrinsic attribute prefix `+` instead of its expression counterpart `*`, as follows:
