...
### By-Name Bound Attributes

As a shorthand for binding dynamic values consisting of a single variable-name (one that is
in essence equal to the name of the attribute to bind to), the bind-by-name operator `&` may be used.

Instead of writing the redundant form of

```component-markup
<container [my-value]="my_value" />
```

one can simply prepend an ampersand to the value-expression, as follows:

```component-markup
<container &my_value />
```

This will have the exact same effect as the longer version shown first. Due to the fact that
identifiers within expressions use `snake_case`, while attribute-names use `kebab-case`,
the conversion from the former to the latter style is handled automatically by the system. The
operand is always denoted as an expression-identifier with underscores, never as a
markup-attribute with hyphens.

For additional convenience, member-access-chains are also supported, where the innermost identifier
serves as the attribute-name to bind to.

```component-markup
<container &first.second.third />
```

In the example above, the value of the expression `l-me: first.second.third` will be bound to the
attribute called `third`.
